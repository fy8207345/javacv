...
public class MyFFmpegFrameRecorder extends FrameRecorder {
    ...
    public static MyFFmpegFrameRecorder createDefault(File f, int w, int h)   throws MyFFmpegFrameRecorder.Exception { return new MyFFmpegFrameRecorder(f, w, h); }
    public static MyFFmpegFrameRecorder createDefault(String f, int w, int h) throws MyFFmpegFrameRecorder.Exception { return new MyFFmpegFrameRecorder(f, w, h); }

    private static MyFFmpegFrameRecorder.Exception loadingException = null;
    public static void tryLoad() throws MyFFmpegFrameRecorder.Exception {
        if (loadingException != null) {
            ...
            try {
                ...
                if (t instanceof MyFFmpegFrameRecorder.Exception) {
                    throw loadingException = (MyFFmpegFrameRecorder.Exception)t;
                } else {
                    throw loadingException = new MyFFmpegFrameRecorder.Exception("Failed to load " + FFmpegFrameRecorder.class, t);
                }
            }
        }
    }
    ...
    static {
        ...
        } catch (MyFFmpegFrameRecorder.Exception ex) { }
    }
    ...
    public synchronized void releaseUnsafe() throws FFmpegFrameRecorder.Exception {
        started = false;
        ...
        if (outputStream != null) {
            try {
                ...
                throw new FFmpegFrameRecorder.Exception("Error on OutputStream.close(): ", ex);
            } finally {
                outputStream = null;
    ...
    static MyFFmpegFrameRecorder.WriteCallback writeCallback = new MyFFmpegFrameRecorder.WriteCallback().retainReference();
    ...
    static MyFFmpegFrameRecorder.SeekCallback seekCallback = new MyFFmpegFrameRecorder.SeekCallback().retainReference();
    ...
    @Override public void start() throws FFmpegFrameRecorder.Exception {
        synchronized (org.bytedeco.ffmpeg.global.avcodec.class) {
            startUnsafe();
        }
    }

    public synchronized void startUnsafe() throws FFmpegFrameRecorder.Exception {
        try (PointerScope scope = new PointerScope()) {

            if (oc != null && !oc.isNull()) {
                throw new FFmpegFrameRecorder.Exception("start() has already been called: Call stop() before calling start() again.");
            }
            ...
            if ((oformat = av_guess_format(format_name, filename, null)) == null) {
                ...
                if ((oformat = av_guess_format(format_name, filename, null)) == null) {
                    throw new FFmpegFrameRecorder.Exception("av_guess_format() error: Could not guess output format for \"" + filename + "\" and " + format + " format.");
                }
            }
            ...
            if (avformat_alloc_output_context2(oc, null, format_name, filename) < 0) {
                throw new FFmpegFrameRecorder.Exception("avformat_alloc_context2() error:\tCould not allocate format context");
            }
        ...
        /* add the audio and video streams using the format codecs
           and initialize the codecs */
            ...
            if (imageWidth > 0 && imageHeight > 0) {
                ...
                } else if (videoCodec != AV_CODEC_ID_NONE) {
                ...
                if ((video_codec = avcodec_find_encoder_by_name(videoCodecName)) == null &&
                    ...
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("avcodec_find_encoder() error: Video codec not found.");
                }
                ...
                if ((video_st = avformat_new_stream(oc, null)) == null) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("avformat_new_stream() error: Could not allocate video stream.");
                }

                if ((video_c = avcodec_alloc_context3(video_codec)) == null) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("avcodec_alloc_context3() error: Could not allocate video encoding context.");
                }

                if (inpVideoStream != null) {
                    if ((ret = avcodec_copy_context(video_st.codec(), inpVideoStream.codec())) < 0) {
                        releaseUnsafe();
                        throw new FFmpegFrameRecorder.Exception("avcodec_copy_context() error:\tFailed to copy context from input to output stream codec context");
                    }
            ...
            if (audioChannels > 0 && audioBitrate > 0 && sampleRate > 0) {
                ...
                if ((audio_codec = avcodec_find_encoder_by_name(audioCodecName)) == null &&
                    ...
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("avcodec_find_encoder() error: Audio codec not found.");
                }
                ...
                if ((audio_st = avformat_new_stream(oc, null)) == null) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("avformat_new_stream() error: Could not allocate audio stream.");
                }

                if ((audio_c = avcodec_alloc_context3(audio_codec)) == null) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("avcodec_alloc_context3() error: Could not allocate audio encoding context.");
                }

                if (inpAudioStream != null && audioChannels > 0) {
                    if ((ret = avcodec_copy_context(audio_st.codec(), inpAudioStream.codec())) < 0) {
                        throw new FFmpegFrameRecorder.Exception("avcodec_copy_context() error " + ret + ": Failed to copy context from input audio to output audio stream codec context\n");
                    }
           ...
           video codecs and allocate the necessary encode buffers */
            if (video_st != null && inpVideoStream == null) {
                ...
                if ((ret = avcodec_open2(video_c, video_codec, options)) < 0) {
                    ...
                    throw new FFmpegFrameRecorder.Exception("avcodec_open2() error " + ret + ": Could not open video codec.");
                }
                av_dict_free(options);
...
//            }
                ...
                if ((picture = av_frame_alloc()) == null) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("av_frame_alloc() error: Could not allocate picture.");
                }
                ...
                if ((picture_buf = new BytePointer(av_malloc(size))).isNull()) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("av_malloc() error: Could not allocate picture buffer.");
                }
            ...
            /* if the output format is not equal to the image format, then a temporary
               picture is needed too. It is then converted to the required output format */
                if ((tmp_picture = av_frame_alloc()) == null) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("av_frame_alloc() error: Could not allocate temporary picture.");
                }

                /* copy the stream parameters to the muxer */
                if ((ret = avcodec_parameters_from_context(video_st.codecpar(), video_c)) < 0) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("avcodec_parameters_from_context() error " + ret + ": Could not copy the video stream parameters.");
                }
            ...
            if (audio_st != null && inpAudioStream == null) {
                ...
                if ((ret = avcodec_open2(audio_c, audio_codec, options)) < 0) {
                    releaseUnsafe();
                    av_dict_free(options);
                    throw new FFmpegFrameRecorder.Exception("avcodec_open2() error " + ret + ": Could not open audio codec.");
                }
                av_dict_free(options);
            ...
            /* ugly hack for PCM codecs (will be removed ASAP with new PCM
               support to compute the input frame size in samples */
                ...
                if ((frame = av_frame_alloc()) == null) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("av_frame_alloc() error: Could not allocate audio frame.");
                }
                ...
                if ((ret = avcodec_parameters_from_context(audio_st.codecpar(), audio_c)) < 0) {
                    releaseUnsafe();
                    throw new FFmpegFrameRecorder.Exception("avcodec_parameters_from_context() error " + ret + ": Could not copy the audio stream parameters.");
                }

                AVDictionary metadata = new AVDictionary(null);
                for (Map.Entry<String, String> e : audioMetadata.entrySet()) {
                    av_dict_set(metadata, e.getKey(), e.getValue(), 0);
                }
            ...
            if (outputStream == null && (oformat.flags() & AVFMT_NOFILE) == 0) {
                ...
                if ((ret = avio_open2(pb, filename, AVIO_FLAG_WRITE, null, options)) < 0) {
                    ...
                    releaseUnsafe();
                    av_dict_free(options);
                    throw new FFmpegFrameRecorder.Exception(errorMsg);
                }
                oc.pb(pb);
            }
    ...
    public synchronized void flush() throws FFmpegFrameRecorder.Exception {
        synchronized (oc) {
    ...
    @Override public void record(Frame frame) throws FFmpegFrameRecorder.Exception {
        record(frame, frame != null && frame.opaque instanceof AVFrame ? ((AVFrame)frame.opaque).format() : AV_PIX_FMT_NONE);
    }
    public synchronized void record(Frame frame, int pixelFormat) throws FFmpegFrameRecorder.Exception {
    ...
    public synchronized boolean recordImage(int width, int height, int depth, int channels, int stride, int pixelFormat, Buffer ... image) throws FFmpegFrameRecorder.Exception {
        try (PointerScope scope = new PointerScope()) {

            if (video_st == null) {
                throw new FFmpegFrameRecorder.Exception("No video output stream (Is imageWidth > 0 && imageHeight > 0 and has start() been called?)");
            }
            if (!started) {
                throw new FFmpegFrameRecorder.Exception("start() was not called successfully!");
            }
            int ret;
               ...
               passing the same picture again */
                ...
                if (pixelFormat == AV_PIX_FMT_NONE) {
                    if ((depth == Frame.DEPTH_UBYTE || depth == Frame.DEPTH_BYTE) && channels == 3) {
                        ...
                        throw new FFmpegFrameRecorder.Exception("Could not guess pixel format of image: depth=" + depth + ", channels=" + channels);
                    }
                }
                ...
                if (video_c.pix_fmt() != pixelFormat || video_c.width() != width || video_c.height() != height) {
                    ...
                    if (img_convert_ctx == null) {
                        throw new FFmpegFrameRecorder.Exception("sws_getCachedContext() error: Cannot initialize the conversion context.");
                    }
...
//        } else {
            ...
            if ((ret = avcodec_send_frame(video_c, image == null || image.length == 0 ? null : picture)) < 0
                ...
                throw new FFmpegFrameRecorder.Exception("avcodec_send_frame() error " + ret + ": Error sending a video frame for encoding.");
            }
            ...
            while (ret >= 0) {
                ...
                if (ret == AVERROR_EAGAIN() || ret == AVERROR_EOF()) {
                    ...
                    throw new FFmpegFrameRecorder.Exception("avcodec_receive_packet() error " + ret + ": Error during video encoding.");
                }
...
//        }
    ...
    public synchronized boolean recordSamples(int sampleRate, int audioChannels, Buffer ... samples) throws FFmpegFrameRecorder.Exception {
        try (PointerScope scope = new PointerScope()) {

            if (audio_st == null) {
                throw new FFmpegFrameRecorder.Exception("No audio output stream (Is audioChannels > 0 and has start() been called?)");
            }
            if (!started) {
                throw new FFmpegFrameRecorder.Exception("start() was not called successfully!");
            }
            ...
            if (samples != null && samples[0] instanceof ByteBuffer) {
                ...
                throw new FFmpegFrameRecorder.Exception("Audio samples Buffer has unsupported type: " + samples);
            }
            ...
            if (samples_convert_ctx == null || samples_channels != audioChannels || samples_format != inputFormat || samples_rate != sampleRate) {
                ...
                if (samples_convert_ctx == null) {
                    throw new FFmpegFrameRecorder.Exception("swr_alloc_set_opts() error: Cannot allocate the conversion context.");
                } else if ((ret = swr_init(samples_convert_ctx)) < 0) {
                    throw new FFmpegFrameRecorder.Exception("swr_init() error " + ret + ": Cannot initialize the conversion context.");
                }
            ...
            while (true) {
                ...
                if ((ret = swr_convert(samples_convert_ctx, plane_ptr2, outputCount, plane_ptr, inputCount)) < 0) {
                    throw new FFmpegFrameRecorder.Exception("swr_convert() error " + ret + ": Cannot convert audio samples.");
                } else if (ret == 0) {
                    break;
                }
                for (int i = 0; samples != null && i < samples.length; i++) {
    ...
    private void writeSamples(int nb_samples) throws FFmpegFrameRecorder.Exception {
    ...
    private boolean record(AVFrame frame) throws FFmpegFrameRecorder.Exception {
        int ret;

        if ((ret = avcodec_send_frame(audio_c, frame)) < 0 && frame != null) {
            throw new FFmpegFrameRecorder.Exception("avcodec_send_frame() error " + ret + ": Error sending an audio frame for encoding.");
        }
        if (frame != null) {
        ...
        while (ret >= 0) {
            ...
            if (ret == AVERROR_EAGAIN() || ret == AVERROR_EOF()) {
                ...
                throw new FFmpegFrameRecorder.Exception("avcodec_receive_packet() error " + ret + ": Error during audio encoding.");
            }
    ...
    private void writePacket(int mediaType, AVPacket avPacket) throws FFmpegFrameRecorder.Exception {
        ...
        synchronized (oc) {
            ...
            if (interleaved && avStream != null) {
                if ((ret = av_interleaved_write_frame(oc, avPacket)) < 0) {
                    throw new FFmpegFrameRecorder.Exception("av_interleaved_write_frame() error " + ret + " while writing interleaved " + mediaTypeStr + " packet.");
                }
            } else {
                if ((ret = av_write_frame(oc, avPacket)) < 0) {
                    throw new FFmpegFrameRecorder.Exception("av_write_frame() error " + ret + " while writing " + mediaTypeStr + " packet.");
                }
            }
        }
    ...
    public synchronized boolean recordPacket(AVPacket pkt) throws FFmpegFrameRecorder.Exception {
        if (ifmt_ctx == null) {
            throw new FFmpegFrameRecorder.Exception("No input format context (Has start(AVFormatContext) been called?)");
        }
        if (!started) {
            throw new FFmpegFrameRecorder.Exception("start() was not called successfully!");
        }
